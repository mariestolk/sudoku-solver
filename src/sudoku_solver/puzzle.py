...
class Puzzle:
    ...
    def __init__(self, value_map, group_map=None):
    ...
    def _initialize_cells(self, value_map, group_map):
        ...
        for row_index, row_values in enumerate(value_map):
            ...
            for col_index, value in enumerate(row_values):
                ...
                cell = create_cell(row_index, col_index, group, value)
                row.append(cell)
            rows.append(row)
        return rows
    ...
    def is_valid_assignment(self, row, column, value):
        ...
        for row in grid:
            ...
            for cell in row:
                if cell.value is not None:
                    row_output += f"[{color_map[cell.group]}]{cell.value}[/] "
                else:
                    row_output += f"[{color_map[cell.group]}].[/] "
            rprint(row_output)

    def print_candidate(self, row, column):
    ...
    def print_candidate_group(self, group):
    ...
    def _reduce_row_candidates(self):
    ...
    def _reduce_column_candidates(self):
    ...
    def _reduce_group_candidates(self):
    ...
    def _reduce_hidden_single(self):
            ...
            for cell in row:
                if cell.value is None:
                    for candidate in cell.candidates:
                        ...
                        candidate_map[candidate].append(cell)

            for candidate, cells in candidate_map.items():
                if len(cells) == 1:
                    cell = cells[0]
                    if len(cell.candidates) > 1:
                        cell.set_candidates([candidate])
    ...
    def _reduce_naked_triples(self):
    ...
    def _remove_candidate_from_line(self, candidate, line, group):
    ...
    def _reduce_pinned_candidate(self):
